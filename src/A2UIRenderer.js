...
const COMPONENTS = {
  // 1. THE HERO STAT (For "Accumulators" and "Timers")
  HeroStat: ({ label, value_key, data }) => {
    const value = data[value_key];
    return (
  ...
  // 2. THE ACTION BUTTON (The "Trigger")
  ...
  // 3. THE CHECKLIST (For "Organizers")
  Checklist: ({ items_key, data, onAction }) => {
    const items = data[items_key] || [];
        ...
        <div className="divide-y divide-slate-50">
          {items.map((item, idx) => (
            <div key={idx} className="p-4 flex items-center gap-3 group hover:bg-slate-50 transition-colors cursor-pointer" 
                 onClick={() => onAction('TOGGLE_CHECKLIST_ITEM', { key: items_key, index: idx })}>
              {item.checked ? 
                <CheckSquare className="text-green-500" size={24} /> : 
                <SquareIcon className="text-slate-300" size={24} />
              }
              <span className={`flex-1 font-medium ${item.checked ? 'text-slate-400 line-through' : 'text-slate-700'}`}>
                {item.label}
              </span>
              <button 
                onClick={(e) => { e.stopPropagation(); onAction('DELETE_CHECKLIST_ITEM', { key: items_key, index: idx }); }}
                className="opacity-0 group-hover:opacity-100 text-slate-300 hover:text-red-500 p-2"
              >
                <X size={18} />
              </button>
            </div>
