...
import { Send, Settings, Activity, Smartphone, MessageSquare, Grid, Plus, Trash2, X } from 'lucide-react';
...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    tool: {
      ...
      nullable: true,
      properties: {
        type: { type: SchemaType.STRING, enum: ["Card"] }, 
        props: { type: SchemaType.OBJECT, properties: { title: { type: SchemaType.STRING } } },
        children: {
          type: SchemaType.ARRAY,
          items: {
            type: SchemaType.OBJECT,
            properties: {
              type: { type: SchemaType.STRING, enum: ["Text", "Gauge", "ButtonRow", "Input"] },
              props: {
                type: SchemaType.OBJECT,
                properties: {
...
- If the user adds data (e.g. "I drank 300ml"), CALCULATE the new total based on the Current App State provided in the context, and return the UPDATED tool.
`;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (activeApp) {
        ...
        INSTRUCTION: If the user wants to update this app, return the FULL tool definition with the new numbers calculated.`;
      }
    ...
    <div className="flex h-screen bg-slate-50 font-sans overflow-hidden relative">
      ...
      <div className={`absolute inset-0 flex flex-col bg-white transition-transform duration-300 ${view === 'chat' ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((m, i) => (
            <div key={i} className={`p-3 rounded-xl max-w-[85%] ${m.role === 'user' ? 'bg-blue-600 text-white self-end ml-auto' : 'bg-slate-100 text-slate-800'}`}>
              {m.text}
            </div>
          ))}
      ...
      <div className={`absolute inset-0 flex flex-col bg-slate-100 transition-transform duration-300 ${view === 'app' ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-4 border-b bg-white z-10 shadow-sm flex justify-between items-center">
          <span className="font-bold text-slate-800 flex items-center gap-2">
            ...
            {activeApp ? activeApp.title : "No App Open"}
          </span>
          <button onClick={() => setView('dock')} className="p-2 hover:bg-slate-100 rounded-full text-blue-600">
            <Grid size={24} />
          </button>
        </div>
        <div className="flex-1 p-6 overflow-y-auto flex items-center justify-center">
          {activeApp ? (
             <div className="w-full max-w-md">
               <A2UIRenderer blueprint={activeApp.blueprint} onAction={(a, p) => alert(`Action: ${a} (Payload: ${p})`)} />
             </div>
          ) : (
            <div className="text-slate-400 text-center">
              <p className="mb-4">No app selected.</p>
              <button onClick={() => setView('dock')} className="text-blue-600 font-bold hover:underline">Open Dock</button>
            </div>
          )}
        </div>
      </div>
      ...
      <div className={`absolute inset-0 bg-slate-800/90 backdrop-blur-md z-40 transition-opacity duration-300 ${view === 'dock' ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="p-6 h-full flex flex-col">
          <div className="flex justify-between items-center text-white mb-8">
            ...
            <button onClick={() => setView(activeApp ? 'app' : 'chat')}><X size={28}/></button>
          </div>
          
          <div className="grid grid-cols-2 gap-4 overflow-y-auto">
            <button onClick={() => { setActiveAppId(null); setView('chat'); }} className="bg-white/10 hover:bg-white/20 border-2 border-dashed border-white/30 rounded-2xl p-6 flex flex-col items-center justify-center gap-3 text-white transition">
              <div className="bg-blue-600 p-3 rounded-full"><Plus size={24}/></div>
      ...
      {settingsOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
          <form onSubmit={saveKey} className="bg-white p-6 rounded-2xl w-80">
            <h2 className="text-lg font-bold mb-4">Gemini API Key</h2>
            <input name="key" type="password" placeholder="AIza..." className="w-full border p-2 rounded mb-4" />
            <button type="submit" className="w-full bg-blue-600 text-white py-2 rounded font-bold">Start</button>
          </form>
        </div>
      )}
    </div>
